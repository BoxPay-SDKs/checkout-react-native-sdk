...
export function getShopperDetails() {
  ...
  const isDeliveryAddressEmpty = address => {
    return Object.values(address).every(value => value === null || value === undefined || value === '');
  };
...
export function generateRandomAlphanumericString(length) {
  const charPool = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'.split('');
  let result = '';
...
export const getPhoneNumberCodeAndCountryName = countryCodeRef => {
  const countryDataJson = countryData;
