...
const SavedCardComponentView = ({savedCards, onProceedForward, errorImage, onClickAddCard, onClickRadio }:SavedCardComponentViewProps) => {
...
const SavedCardRow = ({id, nickName,cardNumber, image, errorImage , isSelected, instrumentTypeValue, onPress, onProceedForward, brandColor, currencySymbol, amount} : SavedCardRowProps) => {
        ...
        <View style={{ paddingVertical: 16, paddingHorizontal: 12, backgroundColor: isSelected ? "#EDF8F4" : "white", borderRadius: isSelected ? 0 : 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View style={{ paddingStart: 12, flex: 1 }}>
                    <Text style={{ fontFamily: 'Poppins-SemiBold', fontSize: 12, color: "#4F4D55" }} onPress={() => onPress(id)} numberOfLines={1} ellipsizeMode='tail'>{nickName}</Text>
                    <Text style={{ fontFamily: 'Poppins-Regular', fontSize: 12, color: "#4F4D55" }} onPress={() => onPress(id)} numberOfLines={1} ellipsizeMode='tail'>{cardNumber}</Text>
                </View>
                <RadioButton
                    value={id}
