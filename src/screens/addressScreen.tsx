...
import { BackHandler, Image, Pressable, SafeAreaView, ScrollView, StatusBar, StyleSheet, Text, View } from 'react-native';
...
const AddressScreen = () => {
    ...
    const [countryNameDropdownData, setCountryNameDropdownData] = useState<{ key: string; value: string }[]>([]);
    const [duplicateCountryNameDropdownData, setDuplicateCountryNameDropdownData] = useState<{ key: string; value: string }[]>([]);
    const [phoneCodeDropDownData, setPhoneCodeDropDownData] = useState<{ key: string; value: string }[]>([])
    const [duplicatePhoneCodeDropdownData, setDuplicatePhoneCodeDropdownData] = useState<{ key: string; value: string }[]>([]);
    const [selectedCountryCode, setSelectedCountryCode] = useState("");
    const selectedPhoneCodeRef = useRef("");
    const [countryNameDropdownDataVisible, setCountryNameDropdownVisible] = useState(false);
    const [phoneCodeDropdownDataVisible, setPhoneCodeDropdownDataVisible] = useState(false)
    ...
    const [secondaryAddressTextField, setSecondaryAddressTextField] = useState<string>("")
    ...
    useEffect(() => {
        const selectedCountry = countryData[selectedCountryCode as keyof typeof countryData];
        if (selectedCountry) {
    ...
    const onChangePostalCode = (updatedText: string) => {
        ...
        if (trimmedText === "") {
            ...
            setIsPinValid(true);
        } else if (selectedPhoneCodeRef.current === "+91" && trimmedText.length < 6) {
    ...
    const onChangeCountryTextField = (updatedText: string) => {
        ...
        if (trimmedText === "") {
            ...
            const filtered = duplicateCountryNameDropdownData
                .filter((key) => key.value.toLowerCase().includes(trimmedText.toLowerCase()))
                .sort();
    ...
    const isAllDetailsValid = (): boolean => {
        ...
        // Email
        const emailTrimmed = safeTrim(emailTextField);
    ...
    useEffect(() => {
        ...
        const selectedCountry = countryData[initialCountryCode as keyof typeof countryData];
        if (selectedCountry) {
            selectedPhoneCodeRef.current = selectedCountry.isdCode || "+91"
        }
        const transformed = Array.from(
            new Map(
                Object.entries(countryData)
                    .map(([key, value]) => [value.fullName, { key, value: value.fullName }])
            ).values()
        ...
        const phoneCodeTransformed = Array.from(
            new Map(
                Object.entries(countryData)
                    .map(([key, value]) => [value.isdCode, { key, value: value.isdCode }])
            ).values()
        ...
        if (
            ...
            const slicedNumber = storedPhoneNumber.slice(selectedPhoneCodeRef.current.length)
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
                    ...
                    {(isShippingEnabled || isFullNameEnabled) && (
                        <>
                            <TextInput
                                mode='outlined'
                                label={
                                    <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: nameTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>Full Name*</Text>
                                }
                                ...
                                style={[styles.textInput, { marginTop: 20, marginHorizontal: 16 }]}
                            ...
                            />
                            {isFullNameValid && (
                                <Text style={{ fontSize: 12, fontFamily: 'Poppins-Regular', color: '#E12121', marginHorizontal:16 }}>{fullNameErrorText}</Text>
                            )}
                        </>
                    )}
                    {(isShippingEnabled || isPhoneNumberEnabled) && (
                        <>
                            <View style={{ flex: 1, flexDirection: "row", marginTop: 20, marginHorizontal: 16, alignItems: 'center' }}>
                                <TextInput
                                    mode="outlined"
                                    ...
                                    style={[styles.textInput, { width: 90, marginEnd: 8, marginTop: 6 }]}
                                    outlineStyle={{
                                        borderRadius: 8,
                                        borderWidth: 1.5,
                                    }}
                                        ...
                                        <TextInput.Icon
                                            icon={() => (
                                                <Image source={require("../assets/images/chervon-down.png")} style={{
                                                    alignSelf: 'center', height: 6, width: 14, marginLeft: 'auto'
                                                }} />
                                            )}
                                        />
                                    }
                                ...
                                <TextInput
                                    mode="outlined"
                                    label={
                                        <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: phoneTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>
                                            Mobile Number*
                                        </Text>
                                    }
                            ...
                            {isPhoneNumberValid && (
                                <Text style={{ fontSize: 12, fontFamily: 'Poppins-Regular', color: '#E12121', marginHorizontal: 16 }}>{mobileNumberErrorText}</Text>
                            )}
                                ...
                                <View style={{
                                    ...
                                    <ScrollView>
                                        {phoneCodeDropDownData.map((item) => (
                                            ...
                                            >
                                                <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: '#2D2B32' }}>
                                                    {item.value}
                                                </Text>
                                            </Pressable>
                                        ))}
                    ...
                    {(isShippingEnabled || isEmailEnabled) && (
                        <>
                            <TextInput
                                mode='outlined'
                                label={
                                    <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: emailTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>Email ID*</Text>
                                }
                            ...
                            />
                            {isEmailValid && (
                                <Text style={{ fontSize: 12, fontFamily: 'Poppins-Regular', color: '#E12121', marginHorizontal:16 }}>{emailIdErrorText}</Text>
                            )}
                        </>
                    )}
                    {isShippingEnabled && (
                        <>
                            <View style={{ flex: 1, flexDirection: "row", marginTop: 20, marginHorizontal: 16,alignItems: 'flex-start' }}>
                                <View style = {{flex:1}}>
                                    <TextInput
                                        mode='outlined'
                                        label={
                                            <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: pincodeTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>ZIP/Postal code*</Text>
                                        }
                                        value={pinTextField || ''}
                                        onChangeText={(it) => {
                                ...
                                <View style = {{flex:1}}>
                                    <TextInput
                                        mode='outlined'
                                        label={
                                            <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: cityTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>City*</Text>
                                        }
                                        value={cityTextField || ''}
                                        onChangeText={(it) => {
                                            onChangeCity(it)
                            ...
                            <TextInput
                                mode='outlined'
                                label={
                                    <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: stateTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>State*</Text>
                                }
                            ...
                            />
                            {isStateValid && (
                                <Text style={{ fontSize: 12, fontFamily: 'Poppins-Regular', color: '#E12121' }}>{stateErrorText}</Text>
                            )}
                            <TextInput
                                mode='outlined'
                                label={
                                    <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: mainAddressTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>House number, Apartment*</Text>
                                }
                            ...
                            />
                            {isMainAddressValid && (
                                <Text style={{ fontSize: 12, fontFamily: 'Poppins-Regular', color: '#E12121' }}>{mainAddressErrorText}</Text>
                            )}
                            <TextInput
                                mode='outlined'
                                label={
                                    <Text style={{ fontSize: 16, fontFamily: 'Poppins-Regular', color: secondaryAddressTextFieldFocused ? '#2D2B32' : '#ADACB0' }}>Area,Colony,Street, Sector</Text>
                                }
            ...
            <View style={{
                ...
                <Pressable style={[styles.buttonContainer, { backgroundColor: checkoutDetails.brandColor }]} onPress={() => {
                    ...
                    <Text style={styles.buttonText}>{isShippingEnabled ? 'Save Address' : 'Save Personal Details'}</Text>
                </Pressable>
            </View>
        </SafeAreaView>
    )
